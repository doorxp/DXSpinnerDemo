...
bool HelloWorld::init()
{
    ...
    pSpinner->selectRow(3, 1, true);
    
    return true;
}


const std::string HelloWorld::titleForRowInComponent(DXSpinner *spinner, ssize_t row, ssize_t inComponent) {
    return std::to_string(inComponent)+":"+std::to_string(row);
}

ssize_t HelloWorld::numberOfRowsInComponent(DXSpinner *spinner, ssize_t inComponent) {
    if (inComponent == 0) {
        return 5;
    }
    else {
...
ssize_t HelloWorld::numberOfComponents(DXSpinner *spinner) {return 4;}
